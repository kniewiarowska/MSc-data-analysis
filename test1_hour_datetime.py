...
def prepare_statistic_from_one_hour(item):
    ...
    date = str(first_date).split("T")[0]
    date_time = str(first_date).split(".")
    temp = date_time[0].replace('T', ' ')
    data = [steps, round(heart_rate_avg, 2), round(raw_intensity_avg, 2), temp, date]
...
final_df = pd.DataFrame(final_data, columns=['steps', 'heart_rate', 'raw_intensity', 'datetime', 'date'])
print(final_df)
final_df.to_csv('data/one_hour_datetime_test1.csv')
